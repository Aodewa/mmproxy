...
func tcpHandleConnection(conn net.Conn, lmInfo *LM_T, logger *zap.Logger) {
	defer conn.Close()
	...
	targetAddr := lmInfo.To6
	if saddr == nil {
		if AddrVersion(conn.RemoteAddr()) == 4 {
			//targetAddr = Opts.TargetAddr4
			targetAddr = lmInfo.To4
		}
...
func TCPListen(listenConfig *net.ListenConfig, lmInfo *LM_T, logger *zap.Logger, errors chan<- error) {
	ctx := context.Background()
	//ln, err := listenConfig.Listen(ctx, "tcp", Opts.ListenAddr)
	ln, err := listenConfig.Listen(ctx, "tcp", lmInfo.From)
	if err != nil {
		logger.Error("failed to bind listener", zap.Error(err))
		errors <- err
		return
	}
	logger.Info(lmInfo.From)
	logger.Info("listening")
	logger.Debug(fmt.Sprintf("%v", lmInfo))

	lmInfo.To4 = strings.Replace(lmInfo.To4, "0.0.0.0", "127.0.0.1", 1)
	for {
		conn, err := ln.Accept()
		if err != nil {
		...
		go tcpHandleConnection(conn, lmInfo, logger)
	}
}
